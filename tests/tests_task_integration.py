...
class TaskTests(TestCase):
    ...
    def test_add_team_task_with_authorization(self):
        ...
        response = client.post(
            ...
            {"id_team": f"{team.pk}", "id_task": f"{task.pk}"},
            format="json",
        )
        self.assertEqual(response.status_code, 201)
    ...
    def test_view_team_s_tasks_with_auth(self):
        ...
        client.force_authenticate(user=user)
        response = client.get(
            "/api/maintenancemanagement/teamtasklist/" + str(team.pk), format="json"
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data, serializer.data)
    ...
    def test_view_team_s_tasks_without_auth(self):
        ...
        task = Task.objects.create(name="task")
        response = client.get(
            f"/api/maintenancemanagement/teamtasklist/{team.pk}", format="json"
        )
        self.assertEqual(response.status_code, 401)
    ...
    def test_view_user_s_tasks_with_auth(self):
        ...
        client.force_authenticate(user=user)
        response = client.get(
            f"/api/maintenancemanagement/usertasklist/{user.pk}", format="json"
        )
        self.assertEqual(response.status_code, 200)
