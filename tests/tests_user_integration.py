...
class UserTests(TestCase):
    ...
    def test_change_user_request_own_detail(self):
        ...
        client.force_authenticate(user=user)
        response = client.put(
            "/api/usersmanagement/users/" + str(user.pk) + "/",
            {"first_name": "Paul"},
            format="json",
        )
    ...
    def test_change_user_request_with_perm(self):
        ...
        pk = response1.data["id"]
        response = client.put(
            "/api/usersmanagement/users/" + str(pk) + "/",
            {"first_name": "Paul"},
            format="json",
        )
        self.assertEqual(response.data["first_name"], "Paul")
    ...
    def test_get_user_permissions_with_perm(self):
        ...
        client.force_authenticate(user=user)
        response = client.get(
            "/api/usersmanagement/users/" + str(pk) + "/get_user_permissions"
        )
    ...
    def test_init_database(self):
        ...
        self.assertEqual(
            "usersmanagement.add_userprofile" in user.get_all_permissions(), True
        )
