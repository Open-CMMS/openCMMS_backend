...
class AuthentificationTests(TestCase):
    ...
    def test_is_connected_with_correct_identifier(self):
        ...
        user = self.set_up()
        client = APIClient()
        response = client.post('/api/usersmanagement/login', 'username=tom&password=truc', content_type='application/x-www-form-urlencoded')
    ...
    def test_is_not_connected_with_incorrect_identifier(self):
        ...
        user = self.set_up()
        client = APIClient()
        response = client.post('/api/usersmanagement/login', 'username=tom&password=machin', content_type='application/x-www-form-urlencoded')
    ...
    def test_is_blocked_cant_connect(self):
        ...
        self.assertEqual(response.data["success"] , 'False')
        self.assertEqual(response.data["is_blocked"] , 'True')
        self.assertEqual(response.data["error"] , "Vous vous êtes trompés trop de fois de mot de passe. Vous êtes bloqués.")
