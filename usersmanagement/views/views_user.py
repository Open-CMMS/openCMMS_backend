...
def user_detail(request, pk):
    ...
    if request.method == "GET":
        if (request.user == user) or (
            request.user.has_perm("usersmanagement.view_userprofile")
        ):
        ...
        if (request.user == user) or (
            request.user.has_perm("usersmanagement.change_userprofile")
        ):
    ...
    if serializer.is_valid():
        response = {
            ...
            "user": UserProfileSerializer(
                UserProfile.objects.get(pk=serializer.data["user_id"])
            ).data,
        }
        return Response(response, status=status.HTTP_200_OK)
    else:
        if str(serializer.errors.get("is_blocked")[0]) == "True":
            send_mail_to_setup_password_after_blocking(
                serializer.errors.get("user_id")[0]
            )
        response = {
            "success": "False",
...
def send_mail_to_setup_password(data):
    ...
    if settings.DEBUG == True:
        url = (
            "https://dev.lxc.pic.brasserie-du-slalom.fr/reset-password?token="
            + token
            + "&username="
            + user.username
        )
        ...
        url = (
            "https://application.lxc.pic.brasserie-du-slalom.fr/reset-password?token="
            + token
            + "&username="
            + user.username
        )
    ...
    email.body = (
        "You have been invited to join openCMMS. \nTo setup your password, please follow this link : "
        + url
    )
    email.to = [user.email]
...
def send_mail_to_setup_password_after_blocking(id):
    ...
    email.subject = "Set Your Password"
    email.body = (
        "You have been blocked after 3 unsuccessful login. \nTo setup your new password, please follow this link : "
        + url
    )
    email.to = [user.email]
